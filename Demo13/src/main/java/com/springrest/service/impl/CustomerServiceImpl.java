...
public class CustomerServiceImpl implements CustomerService {
	...
	public Integer saveCustomerUsingObject(CustomerDTO customerDTO) {
		try {
			...
			handleException(exception); //All exceptions from your RestTemplate calls are subclasses of RestClientException
			return null;
		}
	}
	...
	public Integer saveCustomer(CustomerDTO customerDTO) {
		try {
			...
			ResponseEntity<Integer> response = restTemplate.exchange(targetBaseUrl, HttpMethod.POST, requestEntity,Integer.class);
			return response.getBody();
		} catch (RestClientException exception) {
			handleException(exception);
			return null;
	...
	public CustomerDTO findCustomerUsingEntity(int customerId) throws CustomerNotFoundException {
		try {
			ResponseEntity<CustomerDTO> responseEntity = restTemplate.getForEntity(targetBaseUrl + "/" + customerId,CustomerDTO.class);
	...
	public CustomerDTO findCustomer(int customerId) throws CustomerNotFoundException {
		try {
			ResponseEntity<CustomerDTO> responseEntity = restTemplate.exchange(targetBaseUrl + "/" + customerId,HttpMethod.GET, HttpEntity.EMPTY, CustomerDTO.class);
			return responseEntity.getBody();
		} catch (RestClientException exception) {
			handleNotFoundException(exception, customerId);  
			return null;
	...
	// Rest Template does not support PATCH use HttpComponentsClientHttpRequestFactory()
	// add the dependency httpclient5
	@SuppressWarnings("unused")
	@Override
	public void updateCompanionDetails(int customerId, CompanionDTO companionDTO) throws CustomerNotFoundException {
		try {
			...
			ResponseEntity<Void> responseEntity = restTemplate.exchange(targetBaseUrl + "/" + customerId,HttpMethod.PATCH, requestEntity, Void.class);
		} catch (RestClientException exception) {
			handleNotFoundException(exception, customerId);
		}
	}
