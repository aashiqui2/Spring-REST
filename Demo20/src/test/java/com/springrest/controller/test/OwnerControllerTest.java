package com.springrest.controller.test;
...
import com.springrest.controller.OwnerController;
...
import static com.springrest.util.TestDataUtil.convertToJson;
...
//Test slice for Spring MVC controllers
//- Only loads the specified controller (OwnerController) and related MVC components (like @ControllerAdvice, @JsonComponent, etc.)
//- Does NOT load the full Spring Boot application context (services, repositories, or database beans are not loaded by default)
//- Faster and more focused than @SpringBootTest when you only want to test controller layer
//- If you want to test it with actual server the use  @SpringBootTest
@WebMvcTest(OwnerController.class)
class OwnerControllerTest {

	// Injects MockMvc, which acts like a mini web server for testing the controller.
	// It lets you perform GET, POST, PUT, DELETE requests to your controller methods
	// (@GetMapping, @PostMapping, etc.) without starting a real HTTP server.
	@Autowired
	...
	void testFindOwner_WhenOwnerIdIsValid_ShouldReturnOk() throws Exception {
		...
		// When & Then
		mockMvc.perform(get("/owners/" + inputOwnerId)).andExpect(status().isOk())
	...
	void testGetAllOwnersByPetDateOfBirthBetween_WhenValidDates_ShouldReturnOk() throws Exception {
		...
		when(ownerService.findByAllOwnersByPetDateOfBirthBetween(inputStartDate, inputEndDate))
				.thenReturn(expectedOwnerDTOList);
		// When & Then
	...
	void testGetAllOwnersByPetDateOfBirthBetween_WhenAnyDateIsFuture_ShouldReturnBadRequest() throws Exception {
		...
		mockMvc.perform(get("/owners/pets/dob")
				.param("startDate", inputStartDate.toString())
				.param("endDate",inputEndDate.toString())).andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.[0].message").exists());
	}
	...
	void testGetAllOwnersByPetDateOfBirthBetween_WhenStartDateAfterEndDate_ShouldReturnConflict() throws Exception {
		...
		String expectedMessage = String.format("Start date %s can not be after end date %s.", inputStartDate,
				inputEndDate);
		when(ownerService.findByAllOwnersByPetDateOfBirthBetween(inputStartDate, inputEndDate))
				.thenThrow(new ValidationException(expectedMessage));
		// When & Then
		mockMvc.perform(get("/owners/pets/dob")
				.param("startDate", inputStartDate.toString())
				.param("endDate",inputEndDate.toString()))
